...
if [[ $SCOUTER_ENABLED == true ]]; then
  ...
  total=${#array_name[@]}
  let "total-=1"
  export JAVA_OPTS="$JAVA_OPTS -Dobj_name=${SCOUTER_OBJECTNAME_PREFIX}-${array_name[$total]} -javaagent:$CATALINA_HOME/scouter/agent.java/scouter.agent.jar"
  IFS=
fi
